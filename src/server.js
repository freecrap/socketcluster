...
var options = {
  workers: Number(argv.w) || Number(process.env.SOCKETCLUSTER_WORKERS) || 10,
  brokers: Number(argv.b) || Number(process.env.SOCKETCLUSTER_BROKERS) || 3,
  port: Number(argv.p) || Number(process.env.SOCKETCLUSTER_PORT) || 9500,
  ...
  // set logLevel to 3 to log everything
  logLevel: 1,
  protocol: 'http'
};
